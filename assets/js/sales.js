...
let form = document.getElementById("myForm"),
    ...
    itemUnit = document.getElementById("unit"),
...
function showInfo(){
    document.querySelectorAll('.saleDetails').forEach(info => info.remove())
    getData.forEach((element, index) => {
        let createElement = `<tr class="saleDetails">
            <td>${index+1}</td>
            <td><img src="${element.picture}" alt="" width="50" height="50"></td>
            <td>${element.saleUnit}</td>
            <td>${element.saleUid}</td>
            <td>${element.saleQty}</td>
            <td>${element.salesDate}</td>
            <td>${element.saleDc}</td>
            <td>${element.saleCi}</td>

            <td>
              

                <button class="btn btn-primary" onclick="editInfo(${index}, '${element.picture}', '${element.saleUnit}', '${element.saleUid}', '${element.saleQty}', '${element.salesDate}', '${element.saleDc}', '${element.saleCi}')" data-bs-toggle="modal" data-bs-target="#userForm"><i class="bi bi-pencil-square"></i></button>
...
function editInfo(index, pic, unit, Uid, Qty, sDate, Dc, Ci){
    isEdit = true
    editId = index
    imgInput.src = pic
    itemUnit.value = unit
...
form.addEventListener('submit', (e)=> {
    ...
    const information = {
        ...
        saleUnit: itemUnit.value,
        saleUid: uid.value,
        saleQty: qty.value,
        salesDate: sdate.value,
        saleDc: dc.value,
        saleCi: ci.value
    }
