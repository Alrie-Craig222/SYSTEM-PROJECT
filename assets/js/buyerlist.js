...
document.addEventListener("DOMContentLoaded", function () {
    fetch('http://projectbackend.test/api/buyer')
        .then(response => response.json())
        .then(data => {
            // Assuming your backend returns an array of buyers
            // Iterate over each buyer and display their information
            data.forEach((buyer, index) => {
                let createElement = `<tr class="customerDetails">
                    <td>${index + 1}</td>
                    <td><img src="${buyer.picture}" alt="" width="50" height="50"></td>
                    <td>${buyer.customerName}</td>
                    <td>${buyer.customerAddr}</td>
                    <td>${buyer.customerPhone}</td>
                    <td>${buyer.customerGender}</td>
                    <td>${buyer.customerAi}</td>
                    <td>${buyer.customerCat}</td>
                    <td>
                        <button class="btn btn-success" onclick="readInfo('${buyer.picture}', '${buyer.customerName}', '${buyer.customerAddr}', '${buyer.customerPhone}', '${buyer.customerGender}', '${buyer.customerAi}', '${buyer.customerCat}')" data-bs-toggle="modal" data-bs-target="#readData"><i class="bi bi-eye"></i></button>
                        <button class="btn btn-primary" onclick="editInfo(${index}, '${buyer.picture}', '${buyer.customerName}', '${buyer.customerAddr}', '${buyer.customerPhone}', '${buyer.customerGender}', '${buyer.customerAi}', '${buyer.customerCat}')" data-bs-toggle="modal" data-bs-target="#userForm"><i class="bi bi-pencil-square"></i></button>
...
function showInfo(){
    ...
    getData.forEach((element, index) => {
        let createElement = `<tr class="customerDetails">
            ...
            <td>${element.customerAddr}</td>
            <td>${element.customerPhone}</td>
            <td>${element.customerGender}</td>
            <td>${element.customerAi}</td>
            <td>${element.customerCat}</td>

            <td>
                <button class="btn btn-success" onclick="readInfo('${element.picture}', '${element.customerName}', '${element.customerAddr}', '${element.customerPhone}', '${element.customerGender}', '${element.customerAi}', '${element.customerCat}')" data-bs-toggle="modal" data-bs-target="#readData"><i class="bi bi-eye"></i></button>

                <button class="btn btn-primary" onclick="editInfo(${index}, '${element.picture}', '${element.customerName}', '${element.customerAddr}', '${element.customerPhone}', '${element.customerGender}', '${element.customerAi}', '${element.customerCat}')" data-bs-toggle="modal" data-bs-target="#userForm"><i class="bi bi-pencil-square"></i></button>
...
function readInfo(pic, name, addr, phone, gender, ai, cat){
    document.querySelector('.showImg').src = pic,
    document.querySelector('#showName').value = name,
    document.querySelector("#showAddr").value = addr,
    document.querySelector("#showPhone").value = phone,
    document.querySelector("#showGender").value = gender,
    document.querySelector("#showAi").value = ai,
    document.querySelector("#showCat").value = cat
}


function editInfo(index, pic, name, Addr, Phone, Gender, Ai,  cat){
    isEdit = true
    editId = index
    imgInput.src = pic
    userName.value = name
...
form.addEventListener('submit', (e)=> {
    ...
    const information = {
        ...
        customerAddr: addr.value,
        customerPhone: phone.value,
        customerGender: gender.value,
        customerAi: ai.value,
        customerCat: cat.value
    }
