...
function showInfo(){
    document.querySelectorAll('.supplierDetails').forEach(info => info.remove())
    getData.forEach((element, index) => {
        let createElement = `<tr class="supplierDetails">
            <td>${index+1}</td>
            <td><img src="${element.picture}" alt="" width="50" height="50"></td>
            <td>${element.supplierName}</td>
            <td>${element.supplierAddr}</td>
            <td>${element.supplierPhone}</td>
            <td>${element.supplierEmail}</td>

            <td>
                <button class="btn btn-success" onclick="readInfo('${element.picture}', '${element.supplierName}', '${element.supplierAddr}', '${element.supplierPhone}', '${element.supplierEmail}')" data-bs-toggle="modal" data-bs-target="#readData"><i class="bi bi-eye"></i></button>

                <button class="btn btn-primary" onclick="editInfo(${index}, '${element.picture}', '${element.supplierName}', '${element.supplierAddr}', '${element.supplierPhone}', '${element.supplierEmail}')" data-bs-toggle="modal" data-bs-target="#userForm"><i class="bi bi-pencil-square"></i></button>
...
function readInfo(pic, name, addr, phone, email){
...
function editInfo(index, pic, name, Addr, Phone, Email){
    isEdit = true
    editId = index
    imgInput.src = pic
    userName.value = name
...
form.addEventListener('submit', (e)=> {
    ...
    const information = {
        ...
        supplierName: userName.value,
        supplierAddr: addr.value,
